...
1. "매번 LLM API 호출"과 "개발 시점 코드 생성" 각각의 단점은 무엇이고, KotlinLLM은 그 사이에서 어떤 위치를 잡았나?
2. 처음 실행 시 `asLlm` 지점에서 벌어지는 5단계 흐름을 설명해보라.
3. 이 플러그인이 Kotlin/JVM 전용인 이유를 JDI와 연결해서 설명해보라.
4. 생성된 코드가 처리 못 하는 입력을 만나면 왜 "조용히 틀린 값 반환" 대신 regenerate hook으로 떨어져야 하나?
5. 에이전트에게 grep 도구를 주고 수정 권한을 좁힌 것은 각각 어떤 문제를 막기 위한 설계인가?
